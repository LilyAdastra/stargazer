...
One idempotent step per login: ensure the user's Flyte project (the
sanitized GitHub username) exists. Workspace state used to live on a
per-user PVC, but Flyte v2 doesn't support pod templates on
AppEnvironments yet, so we hold workspace state in each per-notebook
pod's ephemeral storage and push back to the user's GitHub fork on
shutdown.
...
def sanitize_project_id(github_username: str) -> str:
    ...
    Lowercases and replaces invalid characters with hyphens; collapses
    repeated hyphens and strips leading/trailing ones. The whole control
    plane is already named `stargazer`, so no extra prefix is needed.
    """
